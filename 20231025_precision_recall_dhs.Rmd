...
```{r}
# write.table(mpra, paste0("/nfs/jray/screens/ALL_MPRAs/Ho_et_al_analysis/Downstream_Analysis/precision_and_recall_analysis/data/20230825_Tcell_glm_john_mpra_merge_max_columns"), row.names=F, col.names=T, sep="\t", quote=F)
```
...
nrow(subset(mpra, mpra_sig=="Enhancer_Skew"))
```


New and improved loop (does not work)
```{r}
mpra$Skew.logFDR[is.na(mpra$Skew.logFDR)]<- (-1)
mpra$A.logPadj_BH[is.na(mpra$A.logPadj_BH)]<- (-1)
mpra$B.logPadj_BH[is.na(mpra$B.logPadj_BH)]<- (-1)
mpra$A.log2FC[is.na(mpra$A.log2FC)]<- (-1)

system.time(
mpra$mpra_sig <- apply(mpra, 1, function(row) {
  if (row["Skew.logFDR"] > 1 && (row["A.logPadj_BH"] > 1 && row["A.log2FC"] > 1)) {
    return("Enhancer_Skew")
  } else if (row["Skew.logFDR"] <= 1 && (row["A.logPadj_BH"] > 1 && row["A.log2FC"] > 1)) {
