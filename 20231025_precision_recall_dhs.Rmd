...
This is the second version of the precision and recall analysis. This version will use pcre_p for DHS on the x-axis instead of logSkew FDR and the y-axis will be the parameter we modulate. The precision and recall for high PICS (or Open Targets posterior prob) will remain the fill. 
...
Apply John's filter to the data for each time through the loop
```{r, eval=FALSE, echo=FALSE}
...
Revised mpra_sig code which will go into the loop
```{r, eval=FALSE, echo=FALSE}
#Manually temporarily set these FDRs and adjusted p-values that are NA to -1 for easier TRUE/FALSE testing
mpra$Skew.logFDR[is.na(mpra$Skew.logFDR)]<- (-1)
...
mpra$mpra_sig<-"nEnhancer_nSkew"
for(i in 1:nrow(mpra)){
  if(mpra[i,]$Skew.logFDR>1 & (mpra[i,]$max.logPadj_BH>1) ){
    mpra[i,]$mpra_sig<-"Enhancer_Skew"
  }else if(mpra[i,]$Skew.logFDR<=1 & (mpra[i,]$max.logPadj_BH>1) ){
    mpra[i,]$mpra_sig<-"Enhancer_nSkew"
  }
}
...
AdjP_seq <-c(0.001, 0.01, 0.05, 0.1, 0.15, 0.2, 0.25, 0.5, 0.75, 1)
precision.skew.dat <- NULL
for( i in AdjP_seq){
  for(j in skew_seq){
   mpra_subset <- mpra # subset the mpra to only the columns we want. 
   ...
   mpra_subset$maxPadj <- 10^-mpra_subset$max.logPadj_BH
...
########################################################################################################
# GET RID OF THIS LINE AND REPLACE IT WITH A MPRA-SIG LINE WHICH MAKES THE PCRES AND EMVARS DIFFERENT BASED ON I AND J (REPLACE I WITH FOLD CHANGE ADJ P)
mpra2 <- mpra 
...
for(i in 1:nrow(mpra2)){
  if(mpra2[i,]$Skew.logFDR>1 & (mpra2[i,]$max.logPadj_BH>j) ){
    mpra2[i,]$mpra_sig<-"Enhancer_Skew"
  }else if(mpra2[i,]$Skew.logFDR<=1 & (mpra2[i,]$max.logPadj_BH>j) ){
...
########################################################################################################  
   ...
   precision.skew.dat <- rbind(precision.skew.dat, data.frame(i, j, precision, recall, all_true, true_positive, false_positive, false_negative, true_negative, risk_ratio, B, D, pcre_p)) # makes the dataframe used for the heat map
  }
}
```
...
Skew heat map
```{r correct heatmap}
names(precision.skew.dat) <- c("FDR", "Skew", "precision", "recall", "all_true", "true_positive", "false_positive", "false_negative", "true_negative", "risk_ratio", "B", "D", "pcre_p")

ggplot(data.frame(precision.skew.dat), aes(x = as.character(FDR), y = as.character(Skew), fill = pcre_p)) +
  geom_tile() + geom_text(aes(label = pcre_p)) + xlab("FC Log Adj p-value for pCREs") + ylab("Skew") + 
  ggtitle("CD4 DHS enrichment heatmap for Skew") + scale_fill_gradient(low = "gold", high = "firebrick2") 
```
