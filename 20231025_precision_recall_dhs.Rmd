...
Make the MPRA into a Granges file
```{r}
...
This is uploading the dhs data. 
```{r}
#Read in DHS position file
dhs.pos<- read.delim(paste0("/nfs/jray/screens/Mouri_et_al_MPRA/dhs/DHS_Index_and_Vocabulary_hg19_WM20190703.txt"), header=T, stringsAsFactors = F)
dhs.pos<-subset(dhs.pos, select=c(seqname, start, end))
# Create a peak column
...
# This removes two columns from the dataset
rm(mpra.se)
rm(dhs.pos.se)
```
...
for(i in 1:nrow(mpra)){ #Add in DHS peak index for any peak overlapping MPRA SNP
  mpra[i,]$cd4_dhs<-subset(dhs.merge.dat, seqname==mpra[i,]$chr & start<=mpra[i,]$snp_end & end >=mpra[i,]$snp_end)$CD4[1]  
}

for(i in 1:nrow(mpra)){
  if(is.na(mpra[i,]$cd4_dhs)){
  mpra[i,]$cd4_dhs <- 0
}
}
...
This code fixes some sample names in the sample.dat data in the Biosample.name column
```{r}
